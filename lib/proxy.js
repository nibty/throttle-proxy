...
/**
 * Factory method to create new proxy server instance
 *
 * @param {Number} incomingSpeed limit in bytes per second
 * @param {Number} [outgoingSpeed] limit in bytes per second
 * @param {Number} [delay] for all request in ms
 */
function createProxy(incomingSpeed, outgoingSpeed, delay) {
	var proxy = new Proxy(incomingSpeed, outgoingSpeed || incomingSpeed, delay || 0);
